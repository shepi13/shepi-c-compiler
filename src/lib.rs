...
pub fn run_main(args: Args) -> Result<(), CompilerError> {
    ...
    // Compiler passes
    let mut token_iter = lex(&source).or_exit_with(2)?;
    if args.lex {
        println!("Tokens:\n\n{}", token_iter);
        return Ok(());
    }
    let parser_ast = parse(&mut token_iter).or_exit_with(3)?;
    if args.parse {
        println!("Parser AST: {:#?}", parser_ast);
        return Ok(());
    }
    let validated_ast = validate_program(parser_ast).or_exit_with(4, &source)?;
    if args.semantics {
        println!("Resolved AST: {:#?}", validated_ast);
        return Ok(());
    }
    let mut typed_ast = type_check_program(validated_ast).or_exit_with(5, &source)?;
    if args.validate {
